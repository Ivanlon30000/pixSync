        ...
        try:
            if illust["type"] in {"illust", "manga"}:
                if illust["page_count"] == 1:
                    api.download(illust["meta_single_page"]["original_image_url"], path=save_dir)
                else:
...
def main(api:pixiv.AppPixivAPI=None):
    config = load_config()
    token = load_token()
    ...
    for restrict in config["restrict"]:
        mbi = None
        while True:  
            res = api.user_bookmarks_illust(me["user"]["id"], restrict=restrict, max_bookmark_id=mbi)
    ...
    logger.info(f"finish: {len(result['success'])} success, {len(result['fail'])} fail:")
    if result["fail"]:
...
if __name__ == "__main__":
    SESS_PATH = "api.session"
    try:
        with open(SESS_PATH, "rb") as fp:
            api = pickle.load(fp)
    ...
    if isinstance(api, pixiv.AppPixivAPI):
